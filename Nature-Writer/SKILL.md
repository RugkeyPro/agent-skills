...
- 注意公式排版（以期刊规范为准，以下为通用要求）：
  - **斜体**：变量、参数与待估计量（含拉丁/希腊字母）；若需区分向量/矩阵，优先使用**粗斜体**
  - **正体**：函数名与运算符（log, ln, exp, sin, max, min, d, ∂ 等）、数字、单位（SI）、化学元素符号、统计学缩写（CI, SD, ANOVA 等）
  - **上下标**：若上下标是变量/索引（如 *xᵢ*, *kₜ*）用斜体；若为说明性文字/缩写/单位（如 *C*max、*k*cat、*R*0、*T*ref）用正体
  - **常量/专有符号**：数学常量或定义常量（如 *e*, *i*, π）是否用正体以期刊/排版规范为准，保持全文一致
...
8. **陈述边界与敏感性**：只要是模型产生的结果，都存在不确定性与敏感性；若用户未报告（或未做）不确定性/敏感性分析，应提示补充并在表述中使用恰当的限定语
9. **短句为主**:避免过长复合句堆叠（必要时拆句）
10. **避免破折号串联分句**：一般情况下避免使用破折号串联多个分句（如 `sentence1——sentence2——sentence3`）
（may/could/likely）
